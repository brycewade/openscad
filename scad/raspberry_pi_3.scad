...
rfmheight=5;
rfmthickness=2;
rfmpcbthickness=1.25;
rfmpinheight=3.4;
rfmpinlength=5;

module rfmcutout(){
    translate([rfmthickness+rfmpinlength,rfmthickness,0]) cube([rfmlength-rfmthickness-rfmpinlength,rfmwidth,rfmheight]);
    translate([rfmthickness,rfmthickness,rfmheight-rfmpinheight]) cube([rfmpinlength,rfmwidth,rfmpinheight+rfmthickness]);
    translate([rfmthickness,rfmthickness,rfmheight-rfmpcbthickness]) cube([rfmlength,rfmwidth,rfmpcbthickness]);
    translate([0,rfmthickness,0]) cube([rfmthickness+rfmpinlength,rfmthickness/2,rfmheight+rfmthickness]);
    translate([0,rfmwidth+rfmthickness/2,0]) cube([rfmthickness+rfmpinlength,rfmthickness/2,rfmheight+rfmthickness]);
...
module radiomount(){
    difference(){
        cube([rfmlength+2*rfmthickness,rfmwidth+2*rfmthickness,rfmheight+rfmthickness]);
        rfmcutout();
...
module top() {
	...
	module sub() { 
		...
		difference() { 
			...
			union () { // subtract from pcb:
				...
				c_cube(19.9-2*d, pcb[1]+2*tol+2*d, pcb2roof-tol);
    ...
    if (have_nrf24){
        translate([+rfmlength,-rfmwidth/2,pcb2roof]){
            rotate([0,180,0]) radiomount();
        }
    }
    if (part == "demo") 
        color("red") plugs(0); 
